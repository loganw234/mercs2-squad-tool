/* bridge-client.js -- a tiny, dependency-free browser client for the Mercenaries 2 lua-bridge over WebSocket.
 * Connect straight from a web page to the live game -- no Python/relay -- once the bridge speaks WS
 * (see BRIDGE_WEBSOCKET.md, in mercs2-lua-essentials/tools/, for exactly what the bridge needs).
 *
 * Consolidated from mercs2-lua-web-ide and mercs2-webmap's identical, independently-evolved copies of
 * mercs2-lua-essentials/tools/ess-bridge.js (2026-07 audit) -- this is the version that was ahead: it grew
 * a table-aware result serializer neither the original Ess reference copy nor the wiki's spec had yet.
 * This file is the new canonical source; both real consumers should vendor THIS at build time going
 * forward instead of hand-copying between themselves.
 *
 ...
 *
 * Per-tool tuning without forking this file -- opts to the constructor:
 *   resultTimeout      ms to wait for the tagged RESULT line (default 8000)
 *   maxReconnectDelay   ms cap on the reconnect backoff (default 15000; a tool that wants its own
 *                       block-hint/timeout UI to trip sooner -- e.g. ~10 tries in ~25s -- should pass a
 *                       lower cap like 3000 rather than editing this file)
...
(function (root) {
  ...
  //
  // Successful values go through a small game-side serializer instead of bare tostring(), so returning a
  // table shows {x=1, y={...}} rather than "table: 0x...". Depth-capped (3), item-capped (40), cycle-safe,
  // strings %q-quoted, and the final line is newline-escaped -- the single-line transport constraint still
  // holds. Errors stay plain tostring(). Lua 5.0-safe: pairs/type/tostring/table.insert/table.concat/
  // string.format/string.gsub only.
  function wrap(code, tag) {
    var ser =
      ...
      "end " +
      "__ser = function(v) return (string.gsub(s(v, 0, {}), '\\n', '\\\\n')) end end\n";
    return ser +
           "local __ok, __r = pcall(function()\n" + code + "\nend)\n" +
           "Loader.WsSend('" + tag + "' .. (__ok and ('OK\\t' .. __ser(__r)) or ('ERR\\t' .. tostring(__r))))\n";
  }
  ...
  function EssBridge(url, opts) {
    ...
    this._reconnectDelayCap = opts.maxReconnectDelay || 15000;
    ...
    return new Promise(function (resolve, reject) {
      ...
      ws.onclose = function () {
        ...
        if (self.autoReconnect) {
          ...
          self._reconnectDelay = Math.min(self._reconnectDelay * 1.7, self._reconnectDelayCap);
        }
      };
